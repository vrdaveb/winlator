...
Java_com_winlator_x11_CmdEntryPoint_start(JNIEnv *env, __unused jclass cls, jobjectArray args) {
    pthread_t t;
    JavaVM* vm = NULL;
    ...
    if (access("/data/data/com.winlator/files/imagefs/usr/lib/libtermux-exec.so", F_OK) == 0 && !detectTracer()
            && !getenv("XSTARTUP_LD_PRELOAD"))
        setenv("LD_PRELOAD", "/data/data/com.winlator/files/imagefs/usr/lib/libtermux-exec.so", 1);
    ...
    if (!getenv("TMPDIR")) {
        if (access("/tmp", F_OK) == 0)
            setenv("TMPDIR", "/tmp", 1);
        else if (access("/data/data/com.winlator/files/imagefs/usr/tmp", F_OK) == 0)
            setenv("TMPDIR", "/data/data/com.winlator/files/imagefs/usr/tmp", 1);
    }
    ...
    if (!getenv("XKB_CONFIG_ROOT")) {
        ...
        // Termux case
        else if (access("/data/data/com.winlator/files/imagefs/usr/share/X11/xkb", F_OK) == 0)
            setenv("XKB_CONFIG_ROOT", "/data/data/com.winlator/files/imagefs/usr/share/X11/xkb", 1);
    }
...
Java_com_winlator_x11_CmdEntryPoint_windowChanged(JNIEnv *env, __unused jobject cls, jobject surface) {
...
Java_com_winlator_x11_CmdEntryPoint_getXConnection(JNIEnv *env, __unused jobject cls) {
    int client[2];
...
JNIEXPORT jobject JNICALL
Java_com_winlator_x11_CmdEntryPoint_getLogcatOutput(JNIEnv *env, __unused jobject cls) {
    jclass ParcelFileDescriptorClass = (*env)->FindClass(env, "android/os/ParcelFileDescriptor");
    jmethodID adoptFd = (*env)->GetStaticMethodID(env, ParcelFileDescriptorClass, "adoptFd", "(I)Landroid/os/ParcelFileDescriptor;");
    const char *debug = getenv("WINLATOR_X11_DEBUG");
...
JNIEXPORT jboolean JNICALL
Java_com_winlator_x11_CmdEntryPoint_connected(__unused JNIEnv *env, __unused jclass clazz) {
    return conn_fd != -1;
}

JNIEXPORT void JNICALL
Java_com_winlator_x11_CmdEntryPoint_listenForConnections(JNIEnv *env, jobject thiz) {
