...
static void nativeInit(JNIEnv *env, jobject thiz) {
    ...
    if (!Charset.self) {
        ...
        MainActivity.self = FindClassOrDie(env,  "com/winlator/x11/X11Activity");
        MainActivity.getInstance = FindMethodOrDie(env, MainActivity.self, "getInstance", "()Lcom/winlator/x11/X11Activity;", JNI_TRUE);
...
Java_com_winlator_x11_LorieView_renderingInActivity(JNIEnv *env, jobject thiz) {
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    jclass cls = (*env)->FindClass(env, "com/winlator/x11/LorieView");
...
__attribute__((constructor)) static void init(void) {
    pthread_t t;
    if (!strcmp(__progname, "com.winlator.x11"))
