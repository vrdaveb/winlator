...
Java_com_winlator_CmdEntryPoint_start(JNIEnv *env, __unused jclass cls, jobjectArray args) {
    pthread_t t;
    JavaVM* vm = NULL;
    ...
    if (getenv("winlator_X11_DEBUG") && !fork()) {
    ...
    if (access("/data/data/com.winlator/files/usr/lib/libtermux-exec.so", F_OK) == 0)
        setenv("LD_PRELOAD", "/data/data/com.winlator/files/usr/lib/libtermux-exec.so", 1);
    ...
    if (!getenv("TMPDIR")) {
        if (access("/tmp", F_OK) == 0)
            setenv("TMPDIR", "/tmp", 1);
        else if (access("/data/data/com.winlator/files/usr/tmp", F_OK) == 0)
            setenv("TMPDIR", "/data/data/com.winlator/files/usr/tmp", 1);
    }
    ...
    if (!getenv("XKB_CONFIG_ROOT")) {
        ...
        // Termux case
        else if (access("/data/data/com.winlator/files/usr/share/X11/xkb", F_OK) == 0)
            setenv("XKB_CONFIG_ROOT", "/data/data/com.winlator/files/usr/share/X11/xkb", 1);
    }
...
Java_com_winlator_CmdEntryPoint_windowChanged(JNIEnv *env, __unused jobject cls, jobject surface, jstring jname) {
...
Java_com_winlator_CmdEntryPoint_getXConnection(JNIEnv *env, __unused jobject cls) {
    int client[2];
...
JNIEXPORT jobject JNICALL
Java_com_winlator_CmdEntryPoint_getLogcatOutput(JNIEnv *env, __unused jobject cls) {
    jclass ParcelFileDescriptorClass = (*env)->FindClass(env, "android/os/ParcelFileDescriptor");
    jmethodID adoptFd = (*env)->GetStaticMethodID(env, ParcelFileDescriptorClass, "adoptFd", "(I)Landroid/os/ParcelFileDescriptor;");
...
JNIEXPORT jboolean JNICALL
Java_com_winlator_CmdEntryPoint_connected(__unused JNIEnv *env, __unused jclass clazz) {
    return conn_fd != -1;
}
...
static inline void checkConnection(JNIEnv* env) {
    ...
    if ((retval = recv(conn_fd, &b, 1, MSG_PEEK)) <= 0 && errno != EAGAIN) {
        ...
        jclass cls = (*env)->FindClass(env, "com/winlator/CmdEntryPoint");
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_connect(__unused JNIEnv* env, __unused jobject cls, jint fd) {
    if (!Charset.self) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_handleXEvents(JNIEnv *env, jobject thiz) {
    checkConnection(env);
    if (conn_fd != -1) {
        lorieEvent e = {0};
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_startLogcat(JNIEnv *env, __unused jobject cls, jint fd) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_setClipboardSyncEnabled(__unused JNIEnv* env, __unused jobject cls, jboolean enable, __unused jboolean ignored) {
    if (conn_fd != -1) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_sendClipboardAnnounce(JNIEnv *env, __unused jobject thiz) {
    if (conn_fd != -1) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_sendClipboardEvent(JNIEnv *env, __unused jobject thiz, jbyteArray text) {
    if (conn_fd != -1 && text) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_sendWindowChange(__unused JNIEnv* env, __unused jobject cls, jint width, jint height, jint framerate) {
    if (conn_fd != -1) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_sendMouseEvent(__unused JNIEnv* env, __unused jobject cls, jfloat x, jfloat y, jint which_button, jboolean button_down, jboolean relative) {
    if (conn_fd != -1) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_sendTouchEvent(__unused JNIEnv* env, __unused jobject cls, jint action, jint id, jint x, jint y) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_sendStylusEvent(JNIEnv *env, __unused jobject thiz, jfloat x, jfloat y,
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_requestStylusEnabled(JNIEnv *env, __unused jclass clazz, jboolean enabled) {
    if (conn_fd != -1) {
...
JNIEXPORT jboolean JNICALL
Java_com_winlator_LorieView_sendKeyEvent(__unused JNIEnv* env, __unused jobject cls, jint scan_code, jint key_code, jboolean key_down) {
    if (conn_fd != -1) {
...
JNIEXPORT void JNICALL
Java_com_winlator_LorieView_sendTextEvent(JNIEnv *env, __unused jobject thiz, jbyteArray text) {
    if (conn_fd != -1 && text) {
        jsize length = (*env)->GetArrayLength(env, text);
        jbyte *str = (*env)->GetByteArrayElements(env, text, NULL);
        char *p = (char*) str;
...
__attribute__((constructor)) static void init(void) {
    pthread_t t;
    if (!strcmp("com.winlator", __progname))
